...
data_path = '/data/zhuyingying/multimodal_retrieval/dataset'
...
class FashionIQDataset(Dataset):
    ...
    def __init__(self, split: str, dress_types: List[str], mode: str, preprocess: callable):
        ...
        caption_path = data_path + '/fashionIQ_dataset/captions/' + f'cap.{dress_type}.{split}.json'
        for dress_type in dress_types:
        ...
        split_path = data_path + '/fashionIQ_dataset/image_splits/' + f'split.{dress_type}.{split}.json'
        for dress_type in dress_types:
    ...
    def __getitem__(self, index):
        try:
            if self.mode == 'relative':
                ...
                image_path = data_path + '/fashionIQ_dataset/images/' + f"{image_name}.jpg"
...
class CIRRDataset(Dataset):
    ...
    def __init__(self, split: str, mode: str, preprocess: callable):
        ...
        self.triplets: List[dict] = []
        caption_path = data_path + '/cirr_dataset/cirr/captions/' + f'cap.rc2.{split}.json'
        with open(caption_path) as f:
            self.triplets.extend(json.load(f))

        # get a mapping from image name to relative path
        self.image_names: list = []
        split_path = data_path + '/cirr_dataset/cirr/image_splits/' + f'split.rc2.{split}.json'
        with open(split_path) as f:
            self.image_names.extend(json.load(f))
    ...
    def __getitem__(self, index):
        try:
            if self.mode == 'relative':
                ...
                if self.split == 'train':
                    reference_image_path = data_path + '/cirr_dataset/train/' + f"{reference_name}.png"
                    reference_image = self.preprocess(PIL.Image.open(reference_image_path))
                    target_hard_name = self.triplets[index]['target_hard']
                    target_image_path = data_path + '/cirr_dataset/train/' + f"{target_hard_name}.jpg"
                    target_image = self.preprocess(PIL.Image.open(target_image_path))
                ...
                image_name = self.image_names[index]
                image_path = data_path + '/cirr_dataset/train/' + f"{image_name}.png"
