...
from dataloader.pcd_loader import *
from Common.pointoperation import *
...
class Model(object):
    ...
    def train(self):
        ...
        # self.target = obj_2_tensor_or_ply(shape_net[self.opts.choice],self.opts.np,1)
        # self.target = self.target.unsqueeze(0)
        # self.target = self.target.repeat(self.opts.bs,1,1)

        # self.target = load_data1(5,4,self.opts.bs,self.opts.np)
        # self.target = load_data2(16,10,15,self.opts.bs,self.opts.np)
        self.all_target = load_data_all(self.opts.np)
        max_index,_,_ = self.all_target.shape
        ...
        self.fix_z = self.noise_generator(bs = self.opts.bs)
        ...
        for epoch in range(start_epoch, self.opts.max_epoch+1):
            ...
            select = torch.randperm(max_index,dtype=int)
            self.target = torch.index_select(self.all_target,dim = 0,index = select[:self.opts.bs])
            ...
            real_points = Variable(self.target,requires_grad=True)
            ...
            lossD,info = dis_loss(d_real_logit,d_fake_logit,gan=self.opts.gan,noise_label=self.opts.d_flip)
            ...
            lossG,_ = gen_loss(g_real_logit,g_fake_logit,gan=self.opts.gan,noise_label=self.opts.g_flip)
    ...
    def test_once(self,epoch):
        ...
        gen_points4 = normalize_point_cloud(gen_points4)
