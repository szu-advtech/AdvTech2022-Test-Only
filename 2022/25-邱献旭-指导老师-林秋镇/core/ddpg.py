...
class GeneticAgent:
    ...
    def update_parameters(self, batch, p1, p2, critic):
        ...
        p1_q_1, p1_q_2 = critic(state_batch, p1_action)
        p1_q = torch.min(p1_q_1, p1_q_2).flatten()
        p2_q_1, p2_q_2 = critic(state_batch, p2_action)
        p2_q = torch.min(p2_q_1, p2_q_2).flatten()
        eps = 0.0
...
#--------------------------------------------------------------------------#
        ...
        # Construct input interface (Hidden Layer 1)
        self.w_state_l1_1 = nn.Linear(args.state_dim, l1)
        self.w_action_l1_1 = nn.Linear(args.action_dim, l1)
        # Hidden Layer 2
        self.w_l2_1 = nn.Linear(2 * l1, l2)
        if self.args.use_ln: self.lnorm2 = LayerNorm(l2)
        # Out
        self.w_out_1 = nn.Linear(l3, 1)
        self.w_out_1.weight.data.mul_(0.1)
        self.w_out_1.bias.data.mul_(0.1)
        self.to(self.args.device)
...
class DDPG(object):
    ...
    def update_parameters(self, batch):
        ...
        policy_grad_loss = -(self.critic.Q1(state_batch, self.actor.forward(state_batch))).mean()
