...
class Mask_net(nn.Module):
    def __init__(self):
        ...
        self.mask_conv = nn.Conv2d(in_channels=512,out_channels=1,kernel_size=(1,1),bias=True)
        self.Sigmoid = nn.Sigmoid()

        #初始化参数
        # nn.init.normal_(self.mask_conv.weight,mean=0.,std=1.)
        # nn.init.constant_(self.mask_conv.bias,1.)

    def forward(self,feature):
        #feature shape is 512 * 7 * 7
        #mask shape is 1 * 7 * 7
        #mask和feature的数据地址不同
